...
вище спокійно доїде назад через `NotifyTemplates::getType($notifyKey)['твій_ключ']`. Зручно для поведінки
конкретного проекту без форку пакету. Сам пакет такі ключі ігнорує, перевіряє їх ваш admin-контролер, UI чи
імпорт. Два приклади з робочого проекту:
...
public static function typeDefinition(): array
{
    return [
        ...
        // Підписка й шаблон можливі лише для цих ролей. OTP-код завжди йде юзеру, що логіниться, тож шаблон
        // під іншу роль ніколи не використається
        'allowed_roles' => ['client'],
        // Шлеться напряму ($user->notify()) у відповідь на дію самої людини, не через резолвер ролей. У парі з
        // user_configurable = false вимкнена підписка падає на default_channels, і лист однаково йде,
        // тож в адмінці перемикач "активно" показується заблокованим
        'always_sent' => true,
    ];
}
```
...
```php
// Матриця в адмінці: ховати клітинки ролей, яким тип недоступний
$allowed = NotifyTemplates::getType($key)['allowed_roles'] ?? null;
$roleAllowed = $allowed === null || in_array($role, $allowed, true);

// Перемикач підписки: не дати вимкнути те, що не вимикається
abort_if(NotifyTemplates::getType($key)['always_sent'] ?? false, 422, 'Цей тип надсилається завжди');
```

`always_sent` важливий лише для типів, що шлються без резолвера ролей. Тип з `user_configurable = false`, який
іде через `NotifyRoleResolverInterface`, неактивна підписка вимикає по-справжньому: резолвер не повертає для
цієї ролі жодного отримувача.
